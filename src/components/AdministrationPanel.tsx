...
function AdministrationPanel() {
  ...
  // usuwanie zgloszeń niestety mi jeszcze nie wyszło bo ciągle dostaje bład 500 takze sproboje to rozwiazac jeszcze pozniej jak znowu siade do roboty.
  const deleteEnguiry = async (id: any) => {
    if (
      window.confirm("Are you sure you want to delete this client enquiry ?")
    ) {
      await axios.delete(
        `https://strapi-km.herokuapp.com/api/inquiries/${id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  return (
    <>
      <Navbar />
      {/* jeszcze dopracuje, niestety w tym rozwiazaniu wyswietlaja mi sie tylko niektore dane a nie wszystkie nie rozumiem dlaczego skoro w konsoli porownalem nazwy tych danych w wartosci powinny byc przekazywane wszystkei z tych attributes*/}
      <>
        {inquiries.map(({ attributes, id }) => {
          return (
            <>
              <div key={id} className="w-2/6 ml-10">
                <div className="mb-5">
                  <p className="text-indigo-900 text-xs underline underline-offset-2 mb-1">
                    Client's id : {id}
                  </p>
